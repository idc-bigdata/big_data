...
##### The following Code requires installation packages
```{r}
library('dplyr')
library('ggplot2')
library('ggjoy')
...
```{r}
# in this chunk we will use library(dplyr)
...
df3 <- data.frame(x=clean_train$price, y=clean_train$points) 
plot(df2$x, df2$y, cex = 0.5)
#df2 <- data.frame(x=scale(clean_train$price), y=scale(clean_train$points) )
km <- kmeans(df3, centers = 4, nstart = 20)
plot(df3$x, df3$y, cex = 0.5, col=km$cluster, xlab="log(price)", ylab="points")
...
# reading the dataset
reds <- read.csv("https://s3.amazonaws.com/udacity-hosted-downloads/ud651/wineQualityReds.csv")
